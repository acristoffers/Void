 ...
 *  It's not meant to offer every functionality of NSS, only those needed by the application,
 *  while being somewhat configurable to allow backward compatible changes in the future (to
 *  add better security, for example). To create brand new objects, it's recommended to use
 *  the default CryptoParams. When working with files already encrypted before, use the saved
 *  information.
 *
 */
...
/**
 *  \brief Converts a std::string binary blob to its HEX representation, separated by \c separator.
 *
 ...
 *  \brief Wraps Mozillas NSS and eases the encryption process.
 *
 *  Constructs a crypto object for encryption/decryption, deriving a key from \c password and \c salt and
 *  uses the method described in \c params. Initializes its CryptoPrivate with parameters from \c params.
 *  This application always assume that your OS is trustworthy, as there is no way to hide something from a
 *  compromised OS.
 *  Uses Crypto#error to report errors.
 *
 *  \arg \c password The password used to generate the key.
 *  \arg \c salt The salt used to generate the key. This is public information and can be saved unprotected.
 *  \arg \c iv The Initial Vector that will be used during encryption/decryption.
 *  \arg \c params All the parameters that describe what kinds of encryptio, hash, keys, etc will be used by this object.
 *
 *  \see Crypto#encrypt
 *  \see Crypto#decrypt
 *  \see CryptoParams
...
Crypto::Crypto(const std::string password, const std::string salt, const std::string iv, const CryptoParams params)
{
    ...
    SECAlgorithmID *algID = PK11_CreatePBEV2AlgorithmID( _p->keyDeriveFunction, _p->keyDeriveMethod, _p->keyDerivationHash, static_cast<int> (_p->keySize), static_cast<int> (_p->keyDerivationCost), SaltSec.get() );
 ...
 *  Initializes its CryptoPrivate with parameters from \c params.
 *  This application always assume that your OS is trustworthy, as there is no way to hide something from a
 *  compromised OS.
 *
 *  \arg \c key The key used to encrypt/decrypt. Must be at least as long as required by the method choosen in \c params.
 *  \arg \c iv The Initial Vector that will be used during encryption/decryption.
 *  \arg \c params All the parameters that describe what kinds of encryptio, hash, keys, etc will be used by this object.
 *
 *  \see Crypto#encrypt
...
/**
 *  \brief Turns a std::string into a SECItem object and returns its SECItemPtr wrapper.
 *
